...
function evaluateSquare(character,x,y,map,characters,isFinal){
    ...
    for(const occupant of occupants){
        ...
        if(
            getCreatureMovementCost(character,occupant,map) >
            map.rules.feetPerSquare
        ){
            costMultiplier = 2;
        }
...
function getMovementPath(character,x,y,map,characters=[]){
    ...
    while(queue.length > 0){
        ...
        for(let index = 1; index < queue.length; index += 1){
            if(distances.get(key(queue[index].x,queue[index].y)) <
                distances.get(key(queue[bestIndex].x,queue[bestIndex].y))){
                bestIndex = index;
            }
        }
        ...
        if(current.x === x && current.y === y){
            ...
            while(cursorKey !== startKey){
                const node = previous.get(cursorKey);
                path.unshift({x:node.x,y:node.y});
                cursorKey = key(node.from.x,node.from.y);
            }
        ...
        for(let dy = -1; dy <= 1; dy += 1){
            for(let dx = -1; dx <= 1; dx += 1){
                ...
                if(!isFootprintInsideMap(map,character,next.x,next.y)){
                    continue;
                }
                ...
                if(!distances.has(nextKey) || nextDistance < distances.get(nextKey)){
                    ...
                    previous.set(nextKey,{from:current,cost:square.cost});
...
function getMovementCost(character,x,y,map,characters=[]){
    ...
    const path =
        getMovementPath(character,x,y,map,characters);
...
function canMoveTo(character,x,y,map,characters=[]){
    ...
    const cost =
        getMovementCost(character,x,y,map,characters);
